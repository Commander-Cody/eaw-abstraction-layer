...
test(
    ...
    function() assert.is.eaw_object_type(eaw.types.type{name = "DummyType"}) end
)

test(
    "When calling assert.is_not.eaw_type with EaW Type -> should throw error",
    function()
        assert.has_errors(function() assert.is_not.eaw_object_type(eaw.types.type{name = "Dummy"}) end)
    end
)
